$(document).ready(function () {
    ...
    $('#btnSalvar').click(function () {
        var form = $('#formCadastrarTime')[0]; // Obtendo o formulário real
        ...
        if (form.checkValidity() === false) {
            $('#formCadastrarTime').addClass('was-validated');
        } else {
            $.ajax({
                url: '/Time/Create',
                type: 'POST',
                    ...
                    if (response.success) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                $('#modalCadastrarTime').modal('hide');
                                $('#formCadastrarTime')[0].reset();
                                $('#formCadastrarTime').removeClass('was-validated');
                                location.reload();
                            }
                        });
                    } else {
    ...
    $(document).on('click', '.btn-edit', function () {
        var timeId = $(this).data('id'); // Pegar o ID da plataforma
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    $('#funcao').val(response.data.funcao);
                    $('#pkid').val(response.data.pkId);

                    // Mostrar a imagem atual no modal de edição (se houver)
                    if (response.data.caminhoImagem) {
                        $('#previewImagemEditar').attr('src', response.data.caminhoImagem).show();
                    } else {
                        $('#previewImagemEditar').hide(); // Se não houver imagem, esconda o preview
                    }

                    $('#modalEditarTime').modal('show');
                } else {
                    Swal.fire({
                        icon: 'error',
                        title: 'Erro!',
    ...
    $('#btnEditar').click(function () {
        var form = $('#formEditarTime')[0]; // Obtendo o formulário real
        ...
        if (form.checkValidity() === false) {
            $('#formEditarTime').addClass('was-validated');
        } else {
            $.ajax({
                url: '/Time/Edit',
                type: 'POST',
                ...
                success: function (response) {
                    if (response.success) {
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                $('#modalEditarTime').modal('hide');
                                $('#formEditarTime')[0].reset();
                                $('#formEditarTime').removeClass('was-validated');
                                location.reload();
                            }
                        });
                    } else {
